...
if (Test-Path "$tmpDir\icons") {
    & $jar cfm "$PROJ\dist\com.sap.abap.ai.completion_1.0.11.jar" "META-INF\MANIFEST.MF" plugin.xml com\ icons\
} else {
    & $jar cfm "$PROJ\dist\com.sap.abap.ai.completion_1.0.11.jar" "META-INF\MANIFEST.MF" plugin.xml com\
}
Pop-Location

Write-Host ""
Write-Host "=== Verifying JAR contents ==="
& $jar tf "$PROJ\dist\com.sap.abap.ai.completion_1.0.11.jar"

Write-Host ""
Write-Host "File size: $((Get-Item "$PROJ\dist\com.sap.abap.ai.completion_1.0.11.jar").Length / 1KB) KB"
